...
const ShopModal = ({showShop, setShowShop, money, setMoney, updateSidePanel, kanjiList}) => {
    ...
    return (
        <CSSTransition
            ...
            timeout={1000}
            component="div"
            className="p-0 m-0 rounded-r-md overflow-y-scroll absolute left-0 top-0 p-2 bg-gray-300 w-3/5 h-full dark:bg-zinc-900"
        >
            <div>
                <button
                    onClick={() => setShowShop(false)}
                    type="button"
                    className="absolute right-1 top-1 z-50 bg-white rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 dark:hover:bg-gray-500 hover:bg-gray-100 dark:bg-zinc-700 dark:hover:text-gray-200 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
